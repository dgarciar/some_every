...
returns true if that number contains at least one zero. Otherwise, 
...
function hasAZero(arr) {
    ...
    return nums.split('').some(function(value){
            return value === "0";
    });
}
...
If any of the values in the array are not odd, the function 
should return false. 
...
returns true if there are no duplicate values (more than one element 
in the array that has the same value as another). If there are 
...
function hasNoDuplicates(arr) {
    let result = false;
    result = arr.some(function(value,index){
